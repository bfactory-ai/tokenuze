...
pub fn Provider(comptime cfg: ProviderConfig) type {
    ...
    return struct {
        ...
        fn collectEvents(ctx: Context, filters: model.DateFilters, consumer: EventConsumer, progress: ?std.Progress.Node) !void {
            ...
            const TaskFn = struct {
                fn run(args: *const WorkerArgs) void {
                    const shared_ctx = args.shared;
                    ...
                    while (true) {
                        ...
                        const finished = shared_ctx.completed.fetchAdd(1, .acq_rel) + 1;
                        if (shared_ctx.progress) |node| node.setCompletedItems(finished);
                    }
                }
            };
